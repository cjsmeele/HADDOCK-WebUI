 ...
 *
 * - Does a localStorage cache entry exist for the form HTML?
 *   - Yes:
 ...
 *       - Get the component count with getComponentCount()
 ...
 *       - Concatenate all generated instance HTML strings with flattenComponentHTML()
 *     - Load form HTML into #haddockform > .content
 * - finalizeForm()
 ...
 * All components in this list have a `type` property.
 ...
 *
 * Sections component objects contain a `children[]` array which can contain child
 ...
 * loops through the `components[]` tree (we can't loop directly through
 * `componentInstances[]` since it contains a flat list of all instances, while
 * we need to know the tree structure in this step) and the generated html for
 * each instance is concatenated to create the form HTML.
 *
 * This HTML is then inserted into the form container (`#haddockform > .content`).
...
$(function(){
	...
	/**
	 * Create initial instances for all components, respecting their repeat attributes
	 *
	 * @param componentList
	 * @param callback called on completion
	 */
	function instantiateComponents(componentList, callback){
